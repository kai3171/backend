...
class FeSTwo:
    ...
    def get_raw_vectors(self, data, fill_value):
        ...
        methylation = pd.DataFrame(np.full((len(data), len(self.raw)), fill_value), columns=list(self.raw), index=data.index)
...
def FeSTwoTest(file_path,fill_value):
    ...
    fea_vector = FE.get_sqare_raw_vertor(methylation,fill_value)
    # get predicted ages
