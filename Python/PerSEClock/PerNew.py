...
for GEO in geo_check:
    ...
    # 临床数据
    ph_name = pd.read_csv('../../data/pheno/GSE20242_pheno.csv')
    check_labels = ph_name["Age"]

    #表达矩阵(行名是特征，列名是样本)####不读入序号列
    check_features = pd.read_csv('../../data/beta/GSE20242_beta.csv',index_col=0)
    ...
    model1.load_state_dict(torch.load("PerSE_model.pt"))#加载参数
