...
class MongoDBBase:
    ...
    def clumn_find(self, collection_name: str, find_filter: dict, clumn_filter: dict):
        """
        查找多条记录
        :param collection_name: 要查找的集合名
        :param find_filter: 查找条件，字典类型
        ...
        client = self.connect()
        collection = client[self.db_name][collection_name]
        result = collection.find(find_filter,clumn_filter)
        return result

    def array_find(self, collection_name: str , name, value):
        """
        查找多条记录
        :param collection_name: 要查找的集合名
        :param find_filter: 查找条件，字典类型
